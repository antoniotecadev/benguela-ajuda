...
export default function HomeClient() {
    ...
    const [isLoaded, setIsLoaded] = useState({ mural: false, confirmation: false, resolved: false });
    ...
    const [statusFilter, setStatusFilter] = useState<'TODOS' | 'ACTIVOS' | 'RESOLVIDOS'>('TODOS');
    ...
    useEffect(() => {
        ...
        setIsLoaded({ ...isLoaded, mural: false });
        setError(null);
        ...
        const unsubscribe = onSnapshot(
            ...
            (snapshot) => {
                ...
                setIsLoaded({ ...isLoaded, mural: true });
            },
            (err) => {
    ...
    const markAsResolved = async (id: string, contacto: string) => {
        if (!db || !currentUserId) {
            alert('Marcar como resolvido só pode ser feito após autenticação.');
            return;
        }

        const result = prompt('Para marcar como resolvido, por favor insira o número de telefone associado a este pedido ou oferta:');

        if (result !== contacto) {
            return; // Usuário cancelou o prompt
        }

        setIsLoaded({ ...isLoaded, resolved: true });
        ...
        try {
            ...
            setIsLoaded({ ...isLoaded, resolved: false });
        }
    };

    const confirmResolution = async (id: string, contacto: string) => {
        if (!db || !currentUserId) {
            alert('Confirmação de resolução só pode ser feita após autenticação.');
            return;
        }

        const result = prompt('Para confirmar a resolução, por favor insira o número de telefone associado a este pedido ou oferta:');

        if (result !== contacto) {
            return; // Usuário cancelou o prompt
        }

        setIsLoaded({ ...isLoaded, confirmation: true });
        ...
        try {
            ...
            setIsLoaded({ ...isLoaded, confirmation: false });
        }
    };
    ...
    return (
        <>
            ...
            {hasFirebaseConfig && !isLoaded.mural && (
                    ...
                    <form className="mt-4 space-y-3" onSubmit={handleSubmit}>
                        ...
                        <label className="block">
                            <div className="mb-1 flex items-center justify-between">
                                ...
                                <span className="text-xs text-slate-400">{300 - form.descricao.length} caracteres</span>
                            </div>
                            <textarea
                ...
                <section id="mural-board" className="space-y-4">
                    <div className="card-surface rounded-2xl p-4 sm:p-6">
                        <div className="flex flex-col gap-1">
                            <h2 className="text-lg font-semibold text-slate-900">Filtrar mural de Benguela</h2>
                            <div className="mt-1 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                                <strong className="font-semibold">Importante:</strong> cada publicação é fixa. Só o autor ou 2 confirmações independentes podem marcar como resolvida.
                            </div>
                        </div>
                        <div className="mt-3 grid gap-3 sm:grid-cols-3">
                            <label className="block">
                                ...
                                <select
                                    value={typeFilter}
                                    onChange={(event) => setTypeFilter(event.target.value as 'TODOS' | RequestType)}
                                    className="input"
                                >
                    ...
                    {error && <div className="rounded-xl border border-red-300 bg-red-100 p-3 text-sm text-red-900">{error}</div>}

                    <p className="text-xs text-slate-500">Mostrando os registos mais recentes carregados em páginas de {PAGE_SIZE} itens.</p>
                            ...
                            return (
                                <li key={item.id} className="card-surface rounded-2xl p-4 sm:p-5">
                                    ...
                                    <div className="mt-4 grid gap-2 sm:flex sm:flex-wrap">
                                        {callLink && (
                                            <a href={callLink} className="call-btn w-full text-center sm:w-auto">
                                                Ligar
                                            </a>
                                        )}
                                        ...
                                        {!item.resolvido && canResolveItem && (
                                            <button
                                                type="button"
                                                disabled={isLoaded.resolved}
                                                className="resolve-btn w-full sm:w-auto"
                                                onClick={() => markAsResolved(item.id, item.contacto)}
                                            >
                                                Já resolvido
                                            </button>
                                        )}
                                        ...
                                        {!item.resolvido && canConfirmItem && (
                                            <button
                                                type="button"
                                                disabled={isLoaded.confirmation}
                                                className="primary-btn w-full sm:w-auto"
                                                onClick={() => confirmResolution(item.id, item.contacto)}
                                            >
                                                Confirmar
                                            </button>
                                        )}
